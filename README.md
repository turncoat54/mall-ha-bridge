...
- **静态 schema**: 实体集合固定为 `fields` 定义清单; 新字段不自动建实体, 只记日志, 人工定义后重启容器
...
```
示例超市 · 取餐通知          ← 标题: 店铺 + 事件中文
🏪 店铺: 示例超市
📌 状态: 已支付               ← event → 中文(已支付/商家已接单/备餐中/待取餐/已完成)
🧾 订单号: o202601010000001
...
字段以商城实际推送为准(2026-08-21 捕获的真实样例, 脱敏):
...
```json
{
  ...
  "taskStatus": 1,
  "etaMinutes": 1305,
  "occurredAt": "2026-01-01T09:30:17.000000000+08:00"
}
```
...
- 商城仍在开发, 字段可能增减; 未在 `fields` 配置的新字段**不会自动建实体**(静态 schema), 只出现在 bridge 日志(`字段 xxx 未在配置中定义, 忽略`), 在 `fields` 定义后重启容器即可接入
